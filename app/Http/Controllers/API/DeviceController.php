...
class DeviceController extends Controller
{
    ...
    //         'tarif_per_kwh' => 'nullable|numeric|min:0',
    //     ]);
    ...
    //     $validated['daya_va'] = $validated['daya_va'] ?? 900;
    //     $validated['tarif_per_kwh'] = $validated['tarif_per_kwh'] ?? 1444.70;
    ...
    public function getHourlyData(Request $request, Device $device)
    {
        ...
        $hourlyData = DB::table('device_data')
            ...
            ->whereDate('device_data.timestamp', $date)
            ->selectRaw("
        ...
        -- Hitung total kWh secara lebih akurat berdasarkan 30 detik per sampel
        (AVG(watt) / 1000) as kwh_total,
        (AVG(watt) / 1000) * devices.tarif_per_kwh as cost_total
    ")
            ->groupBy('hour_timestamp', 'devices.tarif_per_kwh')
